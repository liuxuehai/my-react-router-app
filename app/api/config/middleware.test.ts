...
describe("MiddlewareConfigManager", () => {
  ...
  describe("默认配置", () => {
    it("应该加载默认的中间件配置", () => {
      ...
      expect(configNames).toContain("signatureAuth");
    });

    it("默认配置应该按正确顺序排列", () => {
      const middleware = manager.getEnabledMiddleware();
      expect(middleware).toHaveLength(3); // Only 3 enabled by default (signatureAuth is disabled)
      ...
      expect(configs[2].name).toBe("signatureAuth");
      expect(configs[3].name).toBe("logger");
    });
  });
  ...
  describe("配置验证", () => {
    it("应该验证中间件名称", () => {
      ...
      expect(() => manager.addMiddleware(config)).toThrow(
    ...
    it("应该验证执行顺序", () => {
      ...
      expect(() => manager.addMiddleware(config)).toThrow(
        "Middleware order must be a non-negative number"
      );
    });
  });
  ...
  describe("配置导入导出", () => {
    ...
    it("导入无效配置应该抛出错误", () => {
      ...
      expect(() => manager.importConfig(invalidJson)).toThrow(
        "Failed to import middleware config"
      );
    });
  });
  ...
  describe("路径模式匹配", () => {
    it("应该支持字符串路径模式", () => {
      ...
      const middleware = manager.getEnabledMiddleware();
      expect(middleware).toHaveLength(4); // 3 default enabled + 1 new
    });
  ...
  describe("重置功能", () => {
    it("应该能够重置为默认配置", () => {
      ...
      expect(manager.getAllConfigs()).toHaveLength(5); // 4 default + 1 custom

      // 重置为默认配置
      manager.resetToDefaults();

      const configs = manager.getAllConfigs();
      expect(configs).toHaveLength(4); // Back to 4 default configs
